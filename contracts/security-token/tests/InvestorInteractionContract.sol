...
contract InvestorInteractionContract is BogusAnnouncement, StandardToken {
  ...
  function InvestorInteractionContract(CheckpointToken _token, MockKYC _KYC, bytes32 name, bytes32 URI, uint256 _type, uint256 _hash, uint256 _blockNumber, bytes32[] _options) BogusAnnouncement(name, URI, _type, _hash) public {
    token = _token;
    if (_blockNumber > 0) {
