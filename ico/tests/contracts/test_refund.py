...
@pytest.fixture
def failed_ico(chain: TesterChain, web3, uncapped_flatprice: Contract, team_multisig, customer, customer_2, preico_starts_at, preico_ends_at, uncapped_flatprice_finalizer) -> Contract:
    ...
    * minimum funding goal: 1200 ether
    ...
    uncapped_flatprice.functions.buy().transact({"from": customer, "value": to_wei(50, "ether")})
    uncapped_flatprice.functions.buy().transact({"from": customer_2, "value": to_wei(70, "ether")})

    assert not uncapped_flatprice.functions.isMinimumGoalReached().call()

    # Make sure customer 1 has some token balance
    time_travel(chain, uncapped_flatprice.functions.endsAt().call() + 1)
    return uncapped_flatprice


@pytest.fixture
def failed_ico_ready_to_refund(chain: TesterChain, failed_ico: Contract, team_multisig) -> Contract:
    """An ICO that did not reach a goal, but has participants.
    ...
    """
    failed_ico.functions.loadRefund().transact({
        "from": team_multisig,
        "value": failed_ico.functions.weiRaised().call()
    })
    return failed_ico
...
def test_can_begin_refund(failed_ico, web3: Web3, team_multisig):
    ...
    assert failed_ico.functions.getState().call() == CrowdsaleState.Failure
    assert failed_ico.functions.weiRaised().call() == to_wei(120, "ether")
...
def test_too_small_refund(failed_ico, web3: Web3, team_multisig, customer):
    ...
    failed_ico.functions.loadRefund().transact({
        "from": team_multisig,
        "value": failed_ico.functions.weiRaised().call() // 2}
    )
    assert failed_ico.functions.getState().call() == CrowdsaleState.Failure

    # Customer tries to reclaim refunds early
    with pytest.raises(TransactionFailed):
        failed_ico.functions.refund().transact({"from": customer})
...
def test_refund(failed_ico_ready_to_refund: Contract, web3: Web3, customer: str, customer_2: str):
    ...
    assert failed_ico_ready_to_refund.functions.loadedRefund().call() == to_wei(120, "ether")
    assert failed_ico_ready_to_refund.functions.getState().call() == CrowdsaleState.Refunding

    # Check that the customer gets money back
    invested_amount = failed_ico_ready_to_refund.functions.investedAmountOf(customer).call()
    ...
    failed_ico_ready_to_refund.functions.refund().transact({"from": customer_2})

    # Everything has been refunded
    assert failed_ico_ready_to_refund.functions.weiRefunded().call() == to_wei(120, "ether")
